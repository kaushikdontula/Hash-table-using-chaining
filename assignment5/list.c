...
void linked_list_free(struct list* list) {
  assert(list);
...
struct node* get_head(struct list* l){
  return l->head;
}
...
struct node* get_next(struct node* t){
  return t->next;
}


void* get_value(struct node* temp){
...
void node_remove(struct list* list, void* value){
  ...
  //initialize the prev to NULL and curr to head
  ...
  while (curr != NULL) {
    ...
    //if the node value matches the value we need to remove
    if (curr->val == value) {
    ...
    //update the prev to current and current to current next
    prev = curr;
    curr = curr->next;
